...
from utils import beep_input, relative_path, sleep_for_n_seconds, sleep_forever,read_file


class UndetectedDriver(Chrome):
...
""")
    ...
    def scroll_element(self, element,how_much=10000):
        ...
        if did_element_scroll:
            ...
            print("Scrolling ",how_much)
            self.execute_script(f"arguments[0].scrollBy(0, {how_much})", element)
            return True
    ...
    def save_screenshot(self, folder,filename=datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + ".png"):
        try:
