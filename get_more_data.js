...
function saveStringToFile(data, filename) {
  ...
  function get_order_online_link(data) {
    let images = data?.[6]?.[75]?.[0]?.[1]?.[2] ?? data?.[6]?.[75]?.[0]?.[0]?.[2] ?? []
    const ls = []
    for (let index = 0; index < images.length; index++) {
      const element = images[index]
  ...
  function get_website(data) {
    ...
    if (url && url.startsWith("/url?q=")) {
      // Quitar '/url?q=' del inicio
      let realUrl = url.substring(7);
      
      // Dividir la cadena de la URL en la primera aparición de '&'
      let splitUrl = realUrl.split('&', 1);
